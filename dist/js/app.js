...
app.config(function($stateProvider, $httpProvider, FluroProvider, $urlRouterProvider, $locationProvider, $analyticsProvider) {
    ...
    function updateSearchOptions() {
        ...
        })), $scope.search.campuses && (filteredContacts = _.filter(filteredContacts, function(contact) {
            var selectedRealmID = $scope.search.campuses;
            return _.some(contact.realms, {
                _id: selectedRealmID
            });
        })), $scope.search.departments && (filteredContacts = _.filter(filteredContacts, function(contact) {
            var selectedRealmID = $scope.search.departments;
            return _.some(contact.realms, {
                _id: selectedRealmID
            });
        })), $scope.search.otherRealms && (filteredContacts = _.filter(filteredContacts, function(contact) {
        ...
        }, []).value(), console.log("groups", $scope.groups), console.log("realms", $scope.realms), 
        $scope.campuses = _.filter($scope.realms, function(realm) {
            return "campus" == realm.definition;
        }), $scope.departments = _.filter($scope.realms, function(realm) {
            return "department" == realm.definition;
        }), $scope.otherRealms = _.filter($scope.realms, function(realm) {
            return !realm.definition;
        }), console.log("campuses", $scope.campuses), $scope.filteredContacts = _.orderBy(filteredContacts, function(contact) {
